-- Migración formal equivalente a db/13_encomiendas.sql.
-- El archivo numerado sigue disponible para instalaciones manuales.
...
create table encomiendas (
  ...
  constraint chk_encomienda_destinatario check (
    ...
    or (tipo = 'ENTRE_DELIVERIES' and cliente_id is null and delivery_destino_id is not null)
  ),
...
create trigger trg_encomiendas_updated before update on encomiendas
for each row execute function fn_touch_updated_at();
create trigger trg_audit_encomiendas after insert or update or delete on encomiendas
...
using (
  ...
  or exists (
    ...
    where d.id = delivery_destino_id and d.sucursal_base_id = auth_sucursal_id()
  )
);
...
begin
  ...
  select * into v_origen from deliveries where id = p_delivery_origen_id and activo;
  ...
  if p_tipo = 'CLIENTE' then
    ...
    if v_direccion is null then raise exception 'Indica la dirección de entrega de la encomienda.'; end if;
    ...
    select * into v_destino from deliveries where id = p_delivery_destino_id and activo;
